...
if DEBUG == 1
    ...
    rho_db = [0]; %transmit power [dB]
    angle2 = deg2rad(0:10:60); % angle of UE2 to BS
    ...
    for PILOT_Situation_cur = PILOT_Situation
        for j = 1:length(angle2)
            for i = 1:length(rho)    
                ...
                i_s = @(s) info_density(s,g_list,g_hat_list,sigma_sq_list,rho(i),n_ul,nbrOfRealizations);
                ...
                %[avg_error_ul(i),  s_val_ul(i)] = searchForCandidateS(error_val,1e-10,1); % Optimize s for the RCUs
                ...
                i_s = @(s) info_density(s,g_list_dl,ghat_dl_list,sigma_sq_list_dl,rho(i),n_dl,nbrOfRealizations);
                error_val = @(s) mean(exp(-max(0, i_s(s) - log(2^b-1)))); % Exact RCUs
                [avg_error_dl(j),  s_val_dl(j)] = searchForCandidateS(error_val,1e-10,1); %Optimization over s 
                %[avg_error_dl(i) ,  s_val_dl(i)] = searchForCandidateS(error_val,1e-10,1);
                disp(['DL error probability for (angle2, M,pilot ,method) = (' num2str(rad2deg(angle2(j))) ',' num2str(M) ',' num2str(PILOT_Situation_cur) ',' char(ESTIMATOR_cur) '): ' num2str(avg_error_dl(j)) ' and s = ' num2str(s_val_dl(j))]);
            ...
            if DEBUG == 1
                ...
                figure(1); hold on;
                semilogy(rad2deg(angle2), avg_error_ul) ;%semilogy(rho_db, avg_error_ul) ;
                set(gca, 'YScale', 'log')
                ...
                figure(2); hold on;
                semilogy(rad2deg(angle2), avg_error_dl) ;%semilogy(rho_db, avg_error_dl) ;
                set(gca, 'YScale', 'log')
                xlabel('Angle of UE 2')
                title('DL')
                ylim([1e-5,1])
...
function [hhat1, hhat2, C1, C2] =  estimateChannel(rho, np, H1, H2, R1, R2, PILOT, PILOT_CONTAMINATION, PILOT_Situation, ESTIMATOR)
...
if strcmp(ESTIMATOR, 'MMSE')
    ...
    C1 = R1 - np*rho*R1Qinv*R1; %covariance of Hhat1,PILOT_Situation == 2时后一项要除2,用MR还用不到
...
for nn = 1:n
    ...
    i_s = i_s -s*abs(v-ghat.*q).^2 + s*abs(v).^2./(1+s*rho*abs(ghat).^2) + log(1+s*rho*abs(ghat).^2); % equ.4 of paper
end
end

...
savefile=1;   
if savefile==1
    filenameTmp ='test_RAcorrChannel_MMSEvsMF_PDRAsim_M96_corr0.6_SINR1_PA0.2_Shift32';%可将仿真关键参数作为文件名
...
function [h_corr, R] = generateChannel(M, UserNum, ASDdeg, nbrOfRealizations)
...
for j = 1:nbrOfRealizations
    for userInd = 1:UserNum         
        R(:,:,j,userInd) = functionRlocalscattering(M,userAngles(:,:,userInd),ASDdeg,1/2); %Covariance matrix of UE
        h(:,j,userInd) = sqrtm(R(:,:,j,userInd))*v(:,j,userInd);
    end
end

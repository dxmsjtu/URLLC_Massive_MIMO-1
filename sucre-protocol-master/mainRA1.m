...
clear all; close all;
filenameTmp ='test_factor';%Method_1_3_shadowFadingStddB factor_ratio_userNum 可将仿真参数作为文件名
...
Save_link1=Save_link_file('mainRA1.m',mkdir_str); %将当前
SUCRe =1; K0values = [100 200 300 500 1000:5000:41000]; %Range of number of inactive UEs in the cell 100 200 300 500 1000:5000:21000
RandomSeed = 0; Kneighboringcells = 10;  SumIndex =0;
Default = 0;
disInterval = 15; % distance interval
nbrOfRAblocks_range =[400];
Chan_range =[1 2];  % 1:Uncorr Rayleigh   2:Corr Rayleigh   3:Los   
M_Appro_range = [0 ];   M_range = [100:50:100];   cellRadius_range =[250:500:250]; pA_range = [1:3:5]/1000;  taup_range = [10:5:10]; Intra_Cell_Ratio_range =[ 10]/10;
PL_exp_range =[38:4:38]/1; Kneighboringcells_range =[10:5:10];   EdgeRatio_range =[60:40:60]/100;    factor_range = [90:10:90]/100;
CompRatio_range =[10:10:10 ]/10; shadowFadingStddB_nlos_range = [8:8];  shadowFadingStddB_los_range = [4:4];
RandomAccess_range =[2 3];
for nbrOfRAblocks = nbrOfRAblocks_range for Chan = Chan_range for M_Appro = M_Appro_range for M = M_range   for taup = taup_range
            for cellRadius = cellRadius_range;  for pA =   pA_range    for Intra_Cell_Ratio =Intra_Cell_Ratio_range    for PL_exp = PL_exp_range   for Kneighboringcells =Kneighboringcells_range  for EdgeRatio = EdgeRatio_range   for factor =  factor_range    for CompRatio = CompRatio_range
...
[K0values,DataAll,DisPower,meanWaitingTimeACBPC,finalWaitingTimes,meanWaitingTimeBaseline]=ACPC_Function(RandomAccess,Para,nbrOfRAblocks);
SumIndex = SumIndex+1;
if SumIndex ==1
    DataMatrix =DataAll;    PowerMatrix = DisPower;
else
...
if plotFig ==1
    filenameTmp ='test_factor';%可将仿真参数作为文件名
    mkdir_str=strcat('.\Simulation\',filenameTmp);
    mkdir(mkdir_str);%一运行就会在当前文件夹下创建simulation文件夹
    ...
    SubCaseNum = 2*2*3;%size(DataMatrix,1) ;    %size(DataMatrix,1);
...
title('Uncorr Rayleigh，\tau_p = 10, 补偿');
ylim([0 10]);

figure
hold on; box on;
Disrange = 0.1*cellRadius_range(1)+disInterval/2:disInterval:cellRadius_range(1)-disInterval/2;
...
% ylim([0 40])
function  [K0values,DataAll,DisPower,meanWaitingTime,finalWaitingTimes,meanWaitingTimeBaseline]=ACPC_Function(RandomAccess,Para,nbrOfRAblocks)
...
powerCounts = zeros(1,disInterval);
transPower = zeros(1,disInterval);
if  Default ==1

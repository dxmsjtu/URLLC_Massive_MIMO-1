...
nbrOfRealizations = 200; %10000 is default value£»
...
for betaInd = 1:length(beta1)    
    ...
    for mInd = 1:length(Mvalues)        
        ...
        %Compute estimate of alpha_t at user 1 using Approx2 in Eq. (36) gammaln(X)=LOG(GAMMA(X))
        ...
        %Apply the retransmission decision rule for each user Eq. (18) or Eq. (19)
        %retransmit1Realizations = rho*beta1(betaInd)*taup>alphaEst1_approx2/2;
        % maximize the average number of resolved collisions or to minimize the risk of false positives (or negatives)
        retransmit1Realizations = rho*beta1(betaInd)*taup>alphaEst1_approx2/2 ;

...
method_range =[ 2 ]; % 1:No Inter.  2: Inter.  3.baseline
nbrOfRAblocks_range =[200];   M_Appro_range = [1];   M_range = [100:50:100];   cellRadius_range =[250:500:250]; 
pA_range = [1:1]/2;  taup_range = [10:5:10]; Intra_Cell_Ratio_range =[10]/10;
PL_exp_range =[38:4:38]/1;   Kneighboringcells_range =[10:5:10];   EdgeRatio_range =[100:10:100]/100;    factor_range = [100:10:100]/100;
CompRatio_range =[10:4:10]/10;   EdgeCompRatio_range =[0 ]/10;   biasDelta_range = [0:0.5:0];   shadowFadingStddB_nlos_range = [8:8];  shadowFadingStddB_los_range = [4:4]; RandomAccess_range =[  3];
...
method_range = Para.method_range; % 1:No Inter. 2: Inter.  3.baseline
...
%                                     end                                           
                                        if betas(userIndices(k)) >0.99
                                          ...
                                          %  >alphaEst_approx2/2+biasDelta*betas(userIndices(k))/sqrt(M);
                                            retransmit(k) =  rho*betas(userIndices(k))*taup*10^(EdgeCompRatio*rhodB_Comp(userIndices(k))/10)...,
                                           >alphaEst_approx2/2+biasDelta*betas(userIndices(k))/sqrt(M);
                                        end                                        
                                        % 原算法

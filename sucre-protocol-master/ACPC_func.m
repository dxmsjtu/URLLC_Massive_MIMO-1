%Initialization
function [K0values,finalWaitingTimesACBPC,meanWaitingTimeACBPC,meanWaitingTimeBaseline,finalWaitingTimesBaseline,meanCollidingUsersNumACBPC] = ACPC_func(SUCRe,Kneighboringcells,nbrOfRAblocks)
...
K0values = [100 250 500 1000:4000:8000];
...
for indProb = 1:length(K0values)
    meanWaitingTimeACBPC(indProb,1) = (([1:maxAttempts maxAttempts])*finalWaitingTimesACBPC(:,indProb,1))/(sum(finalWaitingTimesACBPC(:,indProb,1))+eps);
    meanWaitingTimeACBPC(indProb,2) = (([1:maxAttempts maxAttempts])*finalWaitingTimesACBPC(:,indProb,2))/(sum(finalWaitingTimesACBPC(:,indProb,2))+eps);

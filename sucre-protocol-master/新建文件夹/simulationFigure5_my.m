...
%% [1] Emil Bjornson, Elisabeth de Carvalho, Jesper H. Sorensen, Erik G. Larsson, Petar Popovski, "A Random Access Protocol for Pilot Allocation in Crowded
%Massive MIMO Systems," IEEE Transactions on Wireless Communications,%To appear.
%%Download article: http://arxiv.org/pdf/1604.04248, This is version 1.0 (Last edited: 2017-02-03)%
%License: This code is licensed under the GPLv2 license. If you in any wayuse this code for research that results in publications, please cite our
%original article listed above.
%Initialization
close all; clear all;
%Set the number of Monte-Carlo realizations
nbrOfRealizations = 3; % 2500
...
NMSE_approx1 = zeros(length(Mvalues),1); NMSE_approx2 = zeros(length(Mvalues),1); NMSE_ML = zeros(length(Mvalues),1);
...
%     yt = sqrt(rho*taup)*h1(1:Mvalues(mInd
%     + sqrt(rho*taup)*h2(1:Mvalues(mInd),:) + n(1:Mvalues(mInd),:);
     yt = sqrt(rho*taup)*h1(1:Mvalues(mInd),:) + sqrt(rho*taup)*h2(1:Mvalues(mInd),:) + n(1:Mvalues(mInd),:);
   
    %Compute the precoding vector used by the BS, Eq. (11)
...
% taup = 30;  %Length of RA pilot sequence
    ...
    %  Surrog =exp(gammaln(Mvalues(mInd)+1/2)-gammaln(Mvalues(mInd)));
    %  RealValue =gamma(Mvalues(mInd)+1/2)/gamma(Mvalues(mInd));  first part of (36) of [1]
    %  sum(abs(Surrog-RealValue))
...
hold on; box on;
LineWidth =1.5;
plot(Mvalues,bias_approx1,'k-.','LineWidth',LineWidth);
plot(Mvalues,bias_approx2,'b','LineWidth',LineWidth);
...
plot(Mvalues,NMSE_approx1,'k-.','LineWidth',LineWidth);
plot(Mvalues,NMSE_approx2,'b','LineWidth',LineWidth);
plot(Mvalues,NMSE_ML,'r--','LineWidth',LineWidth);
set(gca,'YScale','log');

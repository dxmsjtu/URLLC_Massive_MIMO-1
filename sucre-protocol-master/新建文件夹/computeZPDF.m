...
%Compute term that appears repeatedly in Eq. (30)
B = real(z)/sqrt(lambda2)*sqrt(lambda1/(lambda1+lambda2));
%Go through the summation in Eq. (30)
pdf = zeros(size(f2ln));
for n = 0:2*M-1    
    pdf(B>0) = pdf(B>0) + exp( gammaln(2*M) - gammaln(2*M-n) - gammaln(n+1) + (2*M-1-n)*log(B(B>0))...,
        + gammaln((n+1)/2) + log( 1 + (-1)^n*gammainc(B(B>0).^2,(n+1)/2) ) + f2ln(B>0) + f1ln(B>0));    
    pdf(B<0) = pdf(B<0) + exp( gammaln(2*M) - gammaln(2*M-n) - gammaln(n+1) + (2*M-1-n)*log(B(B<0))...,
            + gammaln((n+1)/2) + log( 1 - gammainc(B(B<0).^2,(n+1)/2) ) + f2ln(B<0) + f1ln(B<0));    
    if n == (2*M-1)
        pdf(B==0) = pdf(B==0) + exp( gammaln(2*M) - gammaln(2*M-n) - gammaln(n+1) + log(( gamma((n+1)/2) ))...,
            + f2ln(B==0) + f1ln(B==0));
    end    
end

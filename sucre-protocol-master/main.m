...
clear all; close all;
filenameTmp ='BER_N_K';%可将仿真参数作为文件名
...
Save_link1=Save_link_file('main.m',mkdir_str);%将当前
SUCRe =1;
K0values = [100 200 300  500 1000:1000:12000]; %Range of number of inactive UEs in the cell
RandomSeed = 0; Kneighboringcells = 10; nbrOfRAblocks = 200; SumIndex =0;
Kneighboringcells_range =[10]; factor_range = [10:10]/10; EdgeRatio_range =[96:2:100]/100; taup_range = [10 ];  
cellRadius = 250; pA =0.1;  M = 100; Default = 0; shadowFadingStddB = 8;
PL_exp_range =[38:5:38]/1; 
RandomAccess_range =[ 3  ]; CompRatio_range =[10 ]/10; 
Intra_Cell_Ratio_range =[9:11]/10; 
for Kneighboringcells =Kneighboringcells_range for factor =  factor_range for EdgeRatio = EdgeRatio_range for taup = taup_range for PL_exp = PL_exp_range
        for RandomAccess =RandomAccess_range for Intra_Cell_Ratio =Intra_Cell_Ratio_range
            for CompRatio = CompRatio_range
                ...
                [K0values,meanWaitingTimeACBPC,finalWaitingTimes,meanWaitingTimeBaseline]=ACPC_Function(RandomAccess,Para,nbrOfRAblocks);
                SumIndex = SumIndex+1;
                if SumIndex ==1
...
if plotFig ==1
    ...
    SubCaseNum =6 ;    NumofFiles =SubCaseNum;%size(DataMatrix,1);
    
    for i=1:size(DataMatrix,1)/SubCaseNum;
        figure;
        DataMatrix_tmp =DataMatrix((i-1)*NumofFiles+1:i*NumofFiles,:);
        SNR_Matrix_tmp =SNR_Matrix((i-1)*NumofFiles+1:i*NumofFiles,:);
...
function  [K0values,meanWaitingTime,finalWaitingTimes,meanWaitingTimeBaseline]=ACPC_Function(RandomAccess,Para,nbrOfRAblocks)
...
for j = 1:length(neighboringBSs)
    ...
    if  RandomAccess==3
        index = find(POWER>rhoSUCRedB);
        POWER(index) = rhoSUCRedB;
        %lower_bound = rhoSUCRedB*EdgeRatio;         %min(POWER,rhoSUCRedB)

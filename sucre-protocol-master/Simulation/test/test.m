...
clear all; close all;
filenameTmp ='test';%Method_1_3_shadowFadingStddB factor_ratio_userNum 可将仿真参数作为文件名
...
SUCRe =1; K0values = [100 200 300  500 1000:3000:22000]; %Range of number of inactive UEs in the cell
RandomSeed = 0; Kneighboringcells = 10;  SumIndex =0;
Default = 0;
nbrOfRAblocks_range =[100]; M_range = [100:50:100];   cellRadius_range =[250:500:250]; pA_range = [1:1]/3;  taup_range = [10:5:10 ]; Intra_Cell_Ratio_range =[ 10]/10;
PL_exp_range =[38:4:38]/1; Kneighboringcells_range =[10:5:10];   EdgeRatio_range =[80:10:100]/100;    factor_range = [95:5:100]/100;
CompRatio_range =[10:1:10 ]/10; shadowFadingStddB_range = [8:8];  RandomAccess_range =[ 3];
for nbrOfRAblocks = nbrOfRAblocks_range for M = M_range   for taup = taup_range
            for cellRadius = cellRadius_range;  for pA =   pA_range    for Intra_Cell_Ratio =Intra_Cell_Ratio_range    for PL_exp = PL_exp_range   for Kneighboringcells =Kneighboringcells_range  for EdgeRatio = EdgeRatio_range   for factor =  factor_range    for CompRatio = CompRatio_range
                                            for shadowFadingStddB = shadowFadingStddB_range   for RandomAccess =RandomAccess_range
...
Para.Kneighboringcells =Kneighboringcells;  Para.shadowFadingStddB = shadowFadingStddB;
Para.cellRadius =cellRadius ; Para.Default =Default;
...
if plotFig ==1
    filenameTmp ='Method1_3_factor';%可将仿真参数作为文件名
    mkdir_str=strcat('.\Simulation\',filenameTmp);
    mkdir(mkdir_str);%一运行就会在当前文件夹下创建simulation文件夹
    ...
    SubCaseNum = 5*5*3;%size(DataMatrix,1) ;    %size(DataMatrix,1);
    ...
    for method = 1:3
        ...
        for r = 1:nbrOfRAblocks
            ...
            if  RandomAccess==3
                ...
                %          %index = find(POWER>lower_bound);
                %          index_1 = find(10.^(POWER/10)>lower_bound);
                %          POWER(index_1) = 10*log10((10.^(POWER(index_1)/10)).^factor);    rhodB = min(rhoNewdB_R,rhoSUCRedB);
                rhodB = rhoSUCRedB;
            end

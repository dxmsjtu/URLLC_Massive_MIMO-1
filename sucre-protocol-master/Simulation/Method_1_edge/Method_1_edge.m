...
clear all; close all;
filenameTmp ='Method_1_3';%Method_1_3_shadowFadingStddB factor_ratio_userNum 可将仿真参数作为文件名
...
K0values = [1000:3000:51000]; %Range of number of inactive UEs in the cell 100 200 300 500 1000:5000:21000
...
nbrOfRAblocks_range =[1000 4200];M_Appro_range = [0 ];   M_range = [100:50:100];   cellRadius_range =[250:500:250]; pA_range = [1:3]/4000;  taup_range = [10:5:10]; Intra_Cell_Ratio_range =[ 10]/10;
PL_exp_range =[38:4:38]/1; Kneighboringcells_range =[10:5:10];   EdgeRatio_range =[60:10:60]/100;    factor_range = [60:20:100]/100;
CompRatio_range =[14:2:14]/10;EdgeCompRatio_range =[14:2:14]/10;   shadowFadingStddB_nlos_range = [8:8];  shadowFadingStddB_los_range = [4:4]; RandomAccess_range =[1 3];
...
if plotFig ==1
    filenameTmp ='Method_1_edge';%可将仿真参数作为文件名
    mkdir_str=strcat('.\Simulation\',filenameTmp);
    mkdir(mkdir_str);%一运行就会在当前文件夹下创建simulation文件夹
    ...
    SubCaseNum = 3*3*3;%size(DataMatrix,1) ;    %size(DataMatrix,1);
    PlotRatio = 1*4; StepNum = 3; startIndex =2+3*0;
...
%                                             >(alphaEst_approx2-Intra_Cell_Ratio*interCellBias)/2-betas(userIndices(k))/sqrt(M);
                                       ...
                                       % if UserDistance(userIndices(k)) < 1.1*cellRadius/2
                                        if betas(userIndices(k)) >0.99; %< 1.1*cellRadius/2
                                            ...
                                            retransmit(k) =  rho*betas(userIndices(k))*taup*10^(rhodB_Comp(userIndices(k))/10)...,
                                            >(alphaEst_approx2-Intra_Cell_Ratio*interCellBias)/2-betas(userIndices(k))/sqrt(M);
                                        end
                                        % 原算法
%                                         retransmit(k) =  rho*betas(userIndices(k))*taup*10^(CompRatio*rhodB_Comp(userIndices(k))/10)...,

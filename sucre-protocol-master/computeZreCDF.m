...
for k = 0:M-1    
    factor1ln = -(b.^2/lambda2)*(1-lambda1/(lambda1+lambda2)) - k*log(lambda1)-log(pi*lambda2)/2-gammaln(k+1) - ...,
        (k+1/2)*log(1/lambda1+1/lambda2);    
    for n = 0:2*k
        cdf(b>0) = cdf(b>0) - exp( gammaln(2*k+1) - gammaln(2*k+1-n) - gammaln(n+1) + (2*k-n)*log(B(b>0)) + gammaln((n+1)/2) +...,
            log( 1 + (-1)^n*gammainc(B(b>0).^2,(n+1)/2) ) + factor1ln(b>0) - log(2));
        
        cdf(b<0) = cdf(b<0) - exp( gammaln(2*k+1) - gammaln(2*k+1-n) - gammaln(n+1) + (2*k-n)*log(B(b<0)) + gammaln((n+1)/2) +...,
            log( 1 - gammainc(B(b<0).^2,(n+1)/2) ) + factor1ln(b<0) - log(2));

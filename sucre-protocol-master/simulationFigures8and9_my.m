...
% Petar Popovski, "A Random Access Protocol for Pilot Allocation in Crowded Massive MIMO Systems," IEEE Transactions on Wireless Communications,To appear.
%% Download article: http://arxiv.org/pdf/1604.04248
%% License: This code is licensed under the GPLv2 license. If you in any way 
%   use this code for research that results in publications, please cite ouroriginal article listed above.
% [2] Spatial Channel Model for Multiple Input Multiple Output (MIMO) Simulations (Release 13), document TR 25.996, 3GPP, Dec. 2015.
%Initialization
close all; clear all;
UserK = 64; Q_pilot = 54;
Q_pilot_effect =  (UserK*(UserK-1)/2);
(1-1/Q_pilot_effect )^UserK
Q_pilot =  64;% 原始单码方案
(1-1/Q_pilot)^UserK
%Toggle between:
...
includeIntercellInterference = true; % true;  false
...
nbrOfRealizations = 150; %  10000 can provide a reliable result， 200  for test;
%Range of BS antennas
% Mvalues = [1:4 5:5:20 25:25:200];
Mvalues = [ 5:5:20 25:25:200];
% Mvalues = [1 5:5:20 25:50:100];
%  Mvalues = [1 10 13];
...
taup = 10;       %Length of RA pilot sequence
...
rhoUsers = rhoOriginal*ones(1,nbrOfRealizations,maxUsers); %rhoOriginal = 1; Transmit power of UEs in the cell
...
neighboringBSs = 2*cellRadius*sqrt(3)/2 * exp(1i*(0:5)*pi/3);  %scatterplot(neighboringBSs) give a clearer picture of 
...
for k = 1:maxUsers    
    ...
    while sum(notReady)>0        
        ...
        for j = 1:length(neighboringBSs)
            ...
            % size(shadowFadingRealizationsWithinOwnCellUplink(k,:,j))=1*500=1* nbrOfRealizations
            notReady = notReady + (shadowFadingStddB_nlos*shadowFadingRealizations(1,:,k)...,
                - 38*log10(userDistance(1,:,k)) < shadowFadingStddB_nlos*shadowFadingRealizationsIntercellDownlink(j,:,k)...,
                - 38*log10(abs(userLocations(1,:,k)-neighboringBSs(j))) );
        end        
...
for k = 1:K_neighboringcells    
    ...
    while sum(notReady)>0        
        ...
        for j = 1:length(neighboringBSs)            
            %Check which of the users that are served by the right BS
            %size(shadowFadingRealizationsWithinOwnCellUplink(k,:,j)) =1 *
            notReady(j,:) = (shadowFadingStddB_nlos*shadowFadingRealizationsWithinOwnCellUplink(k,:,j)...,
                - 38*log10(abs(userLocationsNeighboring(k,:,j))) < shadowFadingStddB_nlos*shadowFadingRealizationsIntercellUplink(k,:,j)...,
...
for j = 1:length(neighboringBSs)    
    ...
    interCellVarianceDownlink = interCellVarianceDownlink...,
        + qNeighbor*10.^((27 + 98.65 - 34.53 - 38*log10(abs(userLocations-neighboringBSs(j)))+...,
        shadowFadingStddB_nlos*shadowFadingRealizationsIntercellDownlink(j,:,:))/10 );
    interCellVarianceUplink = interCellVarianceUplink + sum(rhoIntercell*10.^( (27 + 98.65 - 34.53 -...,
        38*log10(abs(userLocationsNeighboring(:,:,j) + neighboringBSs(j))) +...,
        shadowFadingStddB_nlos*shadowFadingRealizationsIntercellUplink(:,:,j)  )/10   ),1) ;
end
...
% Table 5.1. Environment parameters of  [2] or (5.2-2) below of  [2]
...
for j = 1:nbrOfRealizations    
    ...
    for userInd = 1:maxUsers        
        %Make the Rayleigh fading channel realizations correlated according to Eq. (43)
...
%Generate noise realizations at the users (12) of [1]
...
%Define matrix to solve probabilities to resolve collisions, false negatives, and false positives
    ...
    for mInd = 1:length(Mvalues)        
        ...
        for cInd = 1:numberOfChannelModels  
            ...
            for k = 1:userInd                
                ...
                %alphaEst1_approx1 = Mvalues(mInd)*q*rho*beta1^2*taup^2./real(z).^2-sigma2;
                %alphaEst1_approx1 = Mvalues(mInd)*q*taup^2*rho(1,:,k).*(betas(1,:,k)./real(z)).^2-sigma2;
...
%%colliding UEs and multiply them with the probability that each of these cases occur.
% %Number of inactive UEs in the cell
% K0 = 10000;
% %Probability that a UE wants to become active in a block
% pA = 0.005;
...
%Define vector to store probabilities of having different number of colliding UEs
probabilities = zeros(length(userValues),length(pP));
for kind = 1:length(userValues)    
    %Compute probability according to the binomial distribution in Eq. (1),(2) 
    %by using logarithms of each term to gain numerical stability  (2) (44) of [1],
    probabilities(kind,:) =  exp( - gammaln(userValues(kind)+1) - gammaln(K0-userValues(kind)+1)...,
        + (userValues(kind)).*log(pA*pP/taup) + (K0-userValues(kind)).*log((1-pA*pP/taup)) ); 
    %first  part of  (1) of [1]:  gammaln(K0+1)
    %second part of  (1) of [1]:  gammaln(userValues(kind)+1) - gammaln(K0-userValues(kind)+1)
    %third  part of  (1) of [1]:  (userValues(kind)).*log(pA*pP/taup) + (K0-userValues(kind)).*log((1-pA*pP/taup))
end
%Use the probabilties of having different number of colliding UEs to obtain
%the probability to resolve collisions, having false negatives and false positives.
...
%Extract the results for M=100 and uncorrelated Rayleigh fading, for different values of the bias delta term
% acceptedUsersM100 = reshape(acceptedUsers(length(Mvalues),1,1,2:end),[length(biasDelta)-1 1]);
% falsePositiveM100 = reshape(remainingCollisions(length(Mvalues),1,1,2:end),[length(biasDelta)-1 1]);
% falseNegativeM100 = reshape(noUsers(length(Mvalues),1,1,2:end),[length(biasDelta)-1 1]);
index_M100 =find(Mvalues==100);
acceptedUsersM100 = reshape(acceptedUsers(index_M100,1,1,2:end),[length(biasDelta)-1 1]);
falsePositiveM100 = reshape(remainingCollisions(index_M100,1,1,2:end),[length(biasDelta)-1 1]);
falseNegativeM100 = reshape(noUsers(index_M100,1,1,2:end),[length(biasDelta)-1 1]);
...
plot(Mvalues,acceptedUsers(:,2,1,1),'b-h','LineWidth',1);
plot(Mvalues,acceptedUsers(:,3,1,1),'r-p','LineWidth',1);
plot(Mvalues,acceptedUsers(:,4,1,1),'g-<','LineWidth',1);
...
ylim([0 1]);
title('Cell size =2500 m');
title('Cell size =250 m');
...
% x=0:0.1:4; sigma = 1;
% func_x =(exp(-(x-1).^2/sigma^2)+exp(-(x-3).^2/sigma^2))./(exp(-(x-1).^2/sigma^2)+exp(-(x-3).^2/sigma^2)+exp(-(x+1).^2/sigma^2)+exp(-(x+3).^2/sigma^2))
% plot(x,func_x)
%  sigma = 0.5;
% func_x1 =(exp(-(x-1).^2/sigma^2)+exp(-(x-3).^2/sigma^2))./(exp(-(x-1).^2/sigma^2)+exp(-(x-3).^2/sigma^2)+exp(-(x+1).^2/sigma^2)+exp(-(x+3).^2/sigma^2))
% plot(x,func_x)
% hold on;
% plot(x,func_x1)
% legend('noise=1','noise=0.5');
% 
% 
% 
% func_x_unnorm =exp(-(x-1).^2/sigma^2)
% plot(x,func_x)
% hold on; 